...
class DirectoryStructure:
    def __init__(
        self, database_dir_name=None, dataset_dir_name=None, test=False
    ) -> None:
        # Parameters
        ...
        if database_dir_name is not None:
            self.database_dir = (
                self.main_dir / self.databases_dir_name / database_dir_name
            )

        if dataset_dir_name is not None:
            self.dataset_dir = self.main_dir / self.datasets_dir_name / dataset_dir_name
            self.phases_dirs = {
                phase: self.dataset_dir / phase for phase in self.phases
            }
            self.phases_csv = {
                phase: f"{phase_dir/phase}.csv"
                for phase, phase_dir in self.phases_dirs.items()
            }

    def database_dict(self) -> dict:
        """Return a python dict with key-value pairs as `{Path(class_directory): list(Path(images)}`."""
        return (
            {
                class_dir: [
                    image for image in class_dir.iterdir() if Path.is_file(image)
                ]
                for class_dir in self.database_dir.iterdir()
                if Path.is_dir(class_dir)
            }
            if Path.is_dir(self.database_dir)
            else {}
        )
    ...
    def dataset_dict(self) -> dict:
        ...
        return {
            phase: {
                ...
                for class_dir in phase_dir.iterdir()
                if Path.is_dir(class_dir)
            }
            for phase, phase_dir in self.phases_dirs.items()
            if Path.is_dir(phase_dir)
        }
