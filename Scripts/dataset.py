...
class Dataset:
    def __init__(self, database_name="Database", dataset_name="Dataset") -> None:
        ...
        # Directories
        self.dirs = DirectoryStructure(
            database_dir_name=database_name, dataset_dir_name=dataset_name
        )
    ...
    def create_dataset_csv(self):
        for phase, phase_data in self.dirs.dataset_dict().items():
            data = [
                (img.name, class_dir.name)
                for class_dir, images in phase_data.items()
                for img in images
            ]
